...
async function getAllMovies(userID = null, sort = '') {
    ...
    query = `select m.id, m.title, m.description, m.submitter, m.created_at, count(case when r.liked then 1 end) as likes, count(case when not r.liked then 1 end) as hates, ${userIDQueryPart} from movies m left join ratings r on m.id = r.movie_id group by m.id ${sort}`
...
async function getMoviesSubmittedBy(submitter, userID = null, sort = '') {
    ...
    query = `select m.id, m.title, m.description, m.submitter, m.created_at, count(case when r.liked then 1 end) as likes, count(case when not r.liked then 1 end) as hates, ${userIDQueryPart} from movies m left join ratings r on m.id = r.movie_id where m.submitter = '${submitter}' group by m.id ${sort}`
